...
if (!($eMail~"active")) do={
  ...
  if ($cfgAddInfos) do={
    ...
    log debug ("   ( $iUsr ) user-comment=[ $[/ip hotspot user get [find name=$iUsr] comment] ]")
    log debug ("   ( $iUsr ) scheduler-comment=[ $[/system scheduler get [find name=$iUsr] comment] ]")
  }
  } on-error={ log error ("   ( $iUsr ) OnLogin ERROR: Update User Comment Info Module") }
  log debug ("   ( $iUsr ) Set User/Scheduler Comment Info Module")
  log debug ("   ( $iUsr ) ===[ TRACKER END ]===")
